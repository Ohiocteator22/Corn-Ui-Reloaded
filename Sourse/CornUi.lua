--[[
    CornUi v1.9.3 — Stable Release
    A lightweight, mobile-compatible Roblox UI library

    FIXES in v1.9.3:
    - Notification sizing fixed (clipped, no full-screen)
    - Export popup fixed (ScrollingFrame + copy button)
    - Command palette "Fire" error fixed
    - Cornelius commands fixed (Core Pack added)
    - Executor function checks added
    - Screenshot command fallback

    Usage:
        local Corn = loadstring(game:HttpGet("url"))()
...
function Library:SaveConfig(name)
    if type(writefile) ~= "function" then
        warn("[MobileUILib] SaveConfig: writefile not available")
        return false
    end
...
function Library:LoadConfig(name, window)
    if type(isfile) ~= "function" or type(readfile) ~= "function" then
        warn("[MobileUILib] LoadConfig: isfile/readfile not available")
        return false
    end
    ...
    if not decodeOk or type(data) ~= "table" then
        warn("[MobileUILib] LoadConfig: malformed config file")
        return false
    end
...
function Library:DeleteConfig(name)
    if type(delfile) ~= "function" or type(isfile) ~= "function" then
        warn("[MobileUILib] DeleteConfig: delfile/isfile not available")
        return false
    end
    local path = configPath(name)
    local existsOk, exists = pcall(isfile, path)
    ...
    local Window = setmetatable({
        ...
        _commands = {
            ...
            ["screenshot"] = function(w)
                local success = false
                if type(GuiService.CaptureScreenshot) == "function" then
                    success = pcall(function() GuiService:CaptureScreenshot() end)
                end
                if success then
                    w:Notify({ Title = "Screenshot", Content = "Captured!", Type = "success" })
                else
                    w:Notify({ Title = "Screenshot", Content = "Not supported in this executor", Type = "warning" })
                end
            end,
            ["help"] = function(w)
                local cmds = {}
...
-- ============================================================
-- NOTIFY (FIXED: Proper sizing with clipping)
-- ============================================================
